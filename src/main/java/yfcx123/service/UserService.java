...
public class UserService {
    ...
    public Mono<User> registerUser(AdminUserDTO userDTO, String password) {
        return checkUserExistence(userDTO)
            ...
            .flatMap(newUser -> authorityRepository.findById(AuthoritiesConstants.USER)
                .map(authority -> {
    ...
    private Mono<Void> checkUserExistence(AdminUserDTO userDTO) {
        return Mono.when(
            userRepository.findOneByLogin(userDTO.getLogin().toLowerCase()).flatMap(existingUser -> {
            ...
            userRepository.findOneByEmailIgnoreCase(userDTO.getEmail()).flatMap(existingUser -> {
                if (!existingUser.isActivated()) {
                    return userRepository.delete(existingUser);
                } else {
